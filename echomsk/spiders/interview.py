        ...
        if not interview_exists:
            ...
            if len(entry['interview']) > 10:
                ...
                for i in entry['interview']:
                    ...
                    interview_item.add_value('guest_title', entry['guest_title'].replace("\xa0", " "))
                    interview_item.add_value('host_name', entry['host_name'].replace("\xa0", " "))
                    interview_item.add_value('index', int(i[0]))
                    interview_item.add_value('speaker', str(i[1]).replace("\xa0", " "))
                    interview_item.add_value('text', str(i[2]).replace("\xa0", " ").encode('utf-8').decode('utf-8'))
